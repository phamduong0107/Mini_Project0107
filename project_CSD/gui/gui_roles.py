...
class GuiSoi(BaseGameGUI):
    ...
    def hanh_dong(self):
        ...
        if muc_tieu:
            # KIỂM TRA ĐỒNG LOẠI VÀ BẢN THÂN
            if muc_tieu == self.game.my_character:
                messagebox.showwarning("Lỗi", "Sói không thể tự cắn chính mình!"); return
            if getattr(muc_tieu, 'phe', '') == "Sói":
                messagebox.showwarning("Lỗi", "Không được cắn đồng loại!"); return

            limit = 2 if getattr(self.game, 'soi_gian_du', False) else 1
